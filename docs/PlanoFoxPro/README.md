...
| `TRAFICO_CRONOGRAMA.md` | `trafico2.scx` (`KeyPress` + `viaje_cambia_cronograma`) + `trafico_cambia_cronograma.scx` | **Diagramación** (teclas F6-F9, Ctrl+F8): las 3 unidades del viaje (U/Pr · U/Cb · U/As), modos diagramador/operador, masivo con preview. La operación MÁS frecuente del circuito (154 cambios/día) | ✅ **construido con andamiaje** (04/08/2026) — `CronogramaDialog`; escritura = día D |
| `TRAFICO_F2_NOVEDADES.md` | `libro_novedad_abm.scx` + `trafico2.scx` (`libro_novedad_alta`) | **F2 — libro de guardia** (1.594 novedades en 2026): el alta usa 5 de las 20 columnas; asunto precargado según el origen; 🐛 la baja del fuente está rota (DELETE comentado y apunta a `agenda`). Cubre también **F1** (ayuda) y **F3** (refresh) | ✅ alta con andamiaje + F1 y F3 activos (04/08/2026) |
| `LIBRO_NOVEDADES.md` | `libro_novedad.scx` · `libro_novedad_envia_correo.scx` · `libro_novedad_parametro.scx` | **El submenú completo** (lista del libro · envío de correos a la lista interna · ABM de destinatarios). Formato exacto de los correos; 🐛 `f_envio` se estampa aunque el envío falle; el INNER JOIN a `chofer` esconde siniestros; baja FÍSICA en las dos tablas | ✅ migrado con andamiaje (19/08/2026) — Combustible y Taller sin migrar |
| `TRAFICO_F4_AVISO.md` | `trafico_hs_aviso.scx` + `trafico2.scx` (`Timer1`) + `funcion.prg` (`formInicioServicio`) | **F4 — alarma por hora del viaje**: setter de `viaje.hs_aviso`, motor de avisos cada 60 s (permiso `C`), popup disparado. Aviso efectivo = `COALESCE(hs_aviso, hs_inicio − parametro.aviso_tiem)`. Corrige el bug de medianoche del FoxPro | ✅ **motor de alarmas ACTIVO** + diálogo F4 con andamiaje (03/08/2026) |
| `TRAFICO_BUSES_MENU.md` | `menu_viaje_vehiculo.mpr` + `trafico2.scx` (`Grid2`) + `trafico_logonear/_deslogonear/_vehiculo_libera.scx` + `vehiculo_logoneo_history/_tarjeta.scx` | **Clic derecho sobre el panel BUSES** (16 acciones + 4 en Ver Datos Extras): GPS, novedad de unidad, combustible, adicionales, OT, logoneo 1º/2º, viáticos, toma franco, ir al viaje, liberar unidad, tarjetas. ⛔ `viaje_log_chofer` sin replicar · 🔴 "Liberar unidad" NO toca `viaje` (bloque comentado) · 🔴 la novedad de unidad guarda el interno como TEXTO en `asunto` | ✅ solo lectura + andamiaje (04/08/2026) |
| `TRAFICO_HISTORIAL.md` | `trafico_historial.scx` | Bitácora `viaje_log` + cabecera de auditoría | ✅ solo lectura |
| `TRAFICO_IR_HORA_ACTUAL.md` | `trafico2.scx` (`ir_hora_actual` + `aplica_filtro`) | Posicionamiento automático de la grilla en la hora actual al abrir. Qué acciones lo disparan y cuáles no. **El FoxPro no resta 2 h** | ✅ portado CENTRADO (29/07/2026) |
...
| `PARAMETROS.md` | **Parámetros Empresa + Generales** (`parametro_empresa.scx` + `parametro.scx`): la fila única de `parametro` (72 columnas). 🐛 3 bugs del fuente (`aviso_mat` no se graba · `dir_mdb`/`intranet` se blanquean solo · `lote_plant` no se graba) · ⚠️ en la misma fila viven los **contadores vivos** del circuito → `parametro` cambia de dueño el día D · `xml_envia` es el interruptor del GPS | ✅ solo lectura + andamiaje (12/08/2026) — `Parametros.razor` |
